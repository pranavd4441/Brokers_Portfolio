...
class PropertyViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        ).prefetch_related("images", "share_links")
        ...
        for idx, file_obj in enumerate(uploaded_files):
            ...
            if async_processing:
                ...
                process_image_async.delay(
                    str(property_obj.id), base64_data, file_obj.name, display_order
                )
            else:
                # 2. Synchronous processing in-request
                try:
                    main_url, thumb_url = process_and_store_image(
                        str(property_obj.id), file_obj
                    )
    ...
    @decorators.action(
        detail=True, methods=["delete"], url_path="images/(?P<image_id>[^/.]+)"
    )
    def delete_image(self, request, pk=None, image_id=None):
        ...
        image_obj.delete()
        log_audit_event(
            request.user, "UPDATE", property_obj, {"deleted_image_id": str(image_id)}
        )
    ...
    def brochure(self, request, pk=None):
        ...
        log_audit_event(
            request.user, "VIEW", property_obj, {"action": "download_brochure"}
        )
