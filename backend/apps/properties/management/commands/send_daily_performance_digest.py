...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for tenant in tenants:
            ...
            top_props = (
                tenant_properties.filter(analytics_events__event_type='PAGE_VIEW', analytics_events__timestamp__gte=yesterday)
            ...
            if top_props.exists():
                ...
                for idx, prop in enumerate(top_props, 1):
                    prop_leads = Lead.objects_unfiltered.filter(property=prop, created_at__gte=yesterday).count()
            ...
            brokers = tenant.users.filter(role__in=['OWNER', 'ADMIN', 'BROKER'], is_active=True).exclude(phone='')
            for broker in brokers:
                try:
                    gateway.send_message(broker.phone, digest)
                    self.stdout.write(self.style.SUCCESS(f"Sent digest to {broker.name} ({broker.phone})"))
                except Exception as e:
                    self.stderr.write(f"Failed to send digest to {broker.name}: {str(e)}")
