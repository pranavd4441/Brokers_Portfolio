...
class PropertyAIService:
    ...
    """
    @staticmethod
    def generate(raw_notes: str, property_type: str = 'APARTMENT', price: str = None, 
                 bhk: str = None, area: str = None, city: str = None) -> dict:
        
        api_key = getattr(settings, 'GEMINI_API_KEY', '')
        if not api_key:
            logger.info("Gemini API key not configured. Using local template fallback.")
            return PropertyAIService._get_fallback_data(raw_notes, property_type, price, bhk, area, city)
...
"""
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=12) as response:  # nosec B310
                ...
                required_fields = ["title", "description", "headlines", "whatsapp_pitches"]
                if all(field in parsed_data for field in required_fields) and len(parsed_data.get("whatsapp_pitches", [])) >= 3:
                    return parsed_data
                else:
            ...
            logger.error(f"Gemini API generation failed: {str(e)}. Using fallback system.")
            return PropertyAIService._get_fallback_data(raw_notes, property_type, price, bhk, area, city)

    @staticmethod
    def _get_fallback_data(raw_notes: str, property_type: str, price: str, 
                            bhk: str, area: str, city: str) -> dict:
        """
