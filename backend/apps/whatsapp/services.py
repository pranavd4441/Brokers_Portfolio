...
class RegexParserService:
    ...
    def parse(text: str) -> dict:
        ...
        elif any(w in text_lower for w in ['apartment', 'flat', 'condo', 'penthouse', 'bhk']):
        ...
        if acre_match:
            ...
            data["square_feet"] = float(acre_match.group(1)) * 43560  # convert to sqft approx or save representation
        ...
        if data["property_type"] or data["area"] or data["bhk"] or data["price"]:
            ...
            data["description"] = f"Beautiful {data['title']} in {city_val}. {text.strip()}{suffix}"
...
class GeminiAudioTranscriptionService:
    ...
    def transcribe(audio_bytes: bytes, mime_type: str) -> str:
        ...
        if not api_key:
            logger.warning("Gemini API key not configured for audio transcription. Using mock fallback.")
...
class BaseWhatsAppGateway:
    def send_message(self, to_number: str, body: str, media_url: str = None, buttons: list = None, list_items: list = None, list_title: str = None) -> bool:
...
class TwilioWhatsAppGateway(BaseWhatsAppGateway):
    ...
    def send_message(self, to_number: str, body: str, media_url: str = None, buttons: list = None, list_items: list = None, list_title: str = None) -> bool:
        ...
        if not account_sid or not auth_token or not from_number:
            ...
            return MockWhatsAppGateway().send_message(to_number, body, media_url, buttons, list_items, list_title)
...
def send_and_log_message(session, body, media_url=None, buttons=None, list_items=None, list_title=None) -> bool:
    """
    Dispatches a message using the active gateway and archives it in the database.
    """
    gateway = get_whatsapp_gateway()
    success = gateway.send_message(session.phone_number, body, media_url, buttons, list_items, list_title)
    ...
    try:
        log_body = body
        if buttons:
            log_body += "\n[Buttons: " + ", ".join(f"{b['title']} ({b['id']})" for b in buttons) + "]"
        if list_items:
            log_body += f"\n[List '{list_title}': " + ", ".join(f"{i['title']} ({i['id']})" for i in list_items) + "]"
