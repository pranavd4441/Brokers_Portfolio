...
class WhatsAppWebhookView(APIView):
    ...
    def _verify_twilio_signature(self, request) -> bool:
        ...
        mac = hmac.new(
            auth_token.encode("utf-8"), full_string.encode("utf-8"), hashlib.sha1
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (
            not is_meta
            and getattr(settings, "WHATSAPP_GATEWAY_PROVIDER", "MOCK").upper()
            == "TWILIO"
        ):
                    ...
                    if audio_id:
                        try:
                            ...
                            with urllib.request.urlopen(
                                req_media, timeout=10
                            ) as res_media:  # nosec B310
                                media_data = json.loads(
                                    res_media.read().decode("utf-8")
                                )
                            ...
                            if media_url:
                                ...
                                with urllib.request.urlopen(
                                    req_file, timeout=15
                                ) as res_file:  # nosec B310
                                    audio_bytes = res_file.read()
                                    body_text = (
                                        GeminiAudioTranscriptionService.transcribe(
                                            audio_bytes, mime_type
                                        )
                                    )
                        except Exception as audio_err:
                            logger.error(
                                f"Failed to fetch/transcribe Meta audio: {str(audio_err)}"
                            )
        ...
        if not from_raw:
            return Response(
                {"detail": "Missing sender details"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            if body_lower in ["cancel", "reset", "exit", "btn_details_cancel"]:
                ...
                session.save()
                send_main_menu(
                    session, user, text_prefix="Listing creation cancelled.\n\n"
                )
            ...
            if body_lower == "btn_main_menu":
                ...
                return Response(
                    {"detail": "Main menu sent."}, status=status.HTTP_200_OK
                )
            ...
            if session.state == "IDLE":
                if body_lower == "menu_create_listing" or body_lower in [
                    "create listing",
                    "create",
                    "new listing",
                    "start",
                ...
                elif body_lower == "menu_my_listings" or body_lower in [
                    "show my listings",
                    "listings",
                    "inventory",
                ]:
                    ...
                    properties = Property.objects.filter(
                        status="SOLD", updated_at__gte=start_of_month
                    )[:10]

                    if not properties.exists():
                        msg = (
                            "You haven't marked any properties as SOLD this month yet."
                        )
                        buttons = [{"id": "btn_main_menu", "title": "Main Menu"}]
                        send_and_log_message(session, msg, buttons=buttons)
                    else:
                    ...
                    log_audit_event(
                        ...
                        instance=prop,
                        changes_payload={
                            "status": {"old": old_status, "new": "SOLD"},
                            "note": "Marked sold via WhatsApp button",
                        },
                    )
                    ...
                    send_and_log_message(
                        session, msg, list_items=list_items, list_title="Edit Listing"
                    )
                    ...
                    session.save()
                    send_and_log_message(
                        session, "Please type the new price (e.g., '60L' or '6000000')."
                    )
                    ...
                    session.save()
                    send_and_log_message(
                        session, "Please type the new description for the property."
                    )
                    ...
                    list_items = [
                        ...
                        {
                            "id": f"status_set_SITE_VISIT_{prop_id}",
                            "title": "Site Visit Scheduled",
                        },
                        {
                            "id": f"status_set_NEGOTIATION_{prop_id}",
                    ...
                    send_and_log_message(
                        session, msg, list_items=list_items, list_title="Select Status"
                    )
                    ...
                    if match:
                        ...
                        log_audit_event(
                            ...
                            changes_payload={
                                ...
                                "note": "Updated status via WhatsApp",
                            },
                        )
                ...
                elif body_lower == "menu_leads" or body_lower in [
                    "new leads today",
                    "leads today",
                    "new leads",
                    "leads",
                    ...
                    if not todays_leads.exists():
                        ...
                        send_and_log_message(
                            session, msg, list_items=list_items, list_title="Leads"
                        )
                    ...
                    if not new_status:
                        ...
                        if matching.count() == 1:
                            ...
                            log_audit_event(
                                ...
                                changes_payload={
                                    "status": {"old": old_status, "new": new_status},
                                    "note": "Updated via WhatsApp command",
                                },
                            )
                ...
                elif body_lower.startswith("leads for ") or body_lower.startswith(
                    "show leads for "
                ):
            ...
            # 9. UPDATING State Machine
                ...
                if field == "price":
                    ...
                    parsed_price = RegexParserService.parse(
                        f"price is {body_text}"
                    ).get("price")
                    if parsed_price:
                        old_price = prop.price
                        ...
                        send_and_log_message(
                            session,
                            f"Could not understand the price '{body_text}'. Please specify like '90 L' or '8500000'.",
                        )
                    ...
                    log_audit_event(
                        ...
                        instance=prop,
                        changes_payload={
                            "description": {"old": old_desc, "new": body_text},
                            "note": "Updated description via WhatsApp",
                        },
                    )
            ...
            # 10. COLLECTING State Machine (Wizard flow)
                    ...
                    for i in range(num_media):
                        ...
                        if is_meta and image_id:
                            try:
                                ...
                                logger.error(
                                    f"Failed to resolve Meta media URL: {str(e)}"
                                )
                        elif not is_meta:
                            media_url = data.get(f"MediaUrl{i}")
                            content_type = data.get(
                                f"MediaContentType{i}", "image/jpeg"
                            )
                        ...
                        if media_url:
                            try:
                                ...
                                main_url, thumb_url = process_and_store_image(
                                    "temp", uploaded_file
                                )
                                ...
                                logger.error(
                                    f"Failed to process WhatsApp media: {str(e)}"
                                )
                    session.save()
                ...
                if step == "AWAITING_PHOTOS":
                    if (
                        body_lower == "btn_skip_photos"
                        or body_lower == "btn_describe_prop"
                    ):
                    ...
                    if body_lower == "btn_details_ok":
                        ...
                        if not price:
                            ...
                            send_and_log_message(
                                session,
                                "I'm still missing a *price*. Please send the price (e.g. '50L' or '5000000').",
                            )
                        else:
                    ...
                    if parsed_price:
                        ...
                        send_and_log_message(
                            session,
                            "Could not understand the price. Please specify like '90 L' or '8500000'.",
                        )
                    ...
                    if body_lower.startswith("wiz_edit_"):
                        ...
                        send_and_log_message(
                            session,
                            f"Please type the new value for *{field.replace('_', ' ').capitalize()}*:",
                        )
                    ...
                    if field == "price":
                        ...
                        if parsed_price:
                            ...
                            send_and_log_message(
                                session,
                                "Could not understand price. Keeping previous value.",
                            )
                    else:
                    ...
                    if body_lower == "btn_select_amenities":
                        ...
                        send_and_log_message(
                            session, msg, list_items=list_items, list_title="Amenities"
                        )
