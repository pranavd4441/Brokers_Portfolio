                ...
                for filename in files:
                    ...
                    try:
                        ...
                        if mtime < cutoff:
                            ...
                            deleted_count += 1
                            self.stdout.write(self.style.SUCCESS(f"Deleted storage temp file: {filepath}"))
                    except Exception as e:
                        self.stderr.write(f"Error checking/deleting storage temp file {filepath}: {str(e)}")
        except Exception as e:
            ...
            try:
                if os.path.isfile(filepath):
                    ...
                    if mtime < cutoff_timestamp:
                        os.remove(filepath)
                        deleted_count += 1
                        self.stdout.write(self.style.SUCCESS(f"Deleted local temp file: {filepath}"))
            except Exception as e:
                self.stderr.write(f"Error deleting local temp file {filepath}: {str(e)}")
