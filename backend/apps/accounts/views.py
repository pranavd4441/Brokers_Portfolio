...
from .models import MFARecoveryCode, MFATicket, UserSession
...
class UserSessionViewSet(viewsets.ModelViewSet):
    ...
    def logout(self, request, pk=None):
        ...
        return Response(
            {"detail": "Session terminated successfully."}, status=status.HTTP_200_OK
        )
    ...
    def logout_other(self, request):
        ...
        current_session = UserSession.objects.filter(
            user=request.user, ip_address=ip, user_agent=ua, is_active=True
        ).first()
    ...
    def logout_all(self, request):
        ...
        return Response(
            {"detail": "All active sessions logged out successfully."},
            status=status.HTTP_200_OK,
        )
