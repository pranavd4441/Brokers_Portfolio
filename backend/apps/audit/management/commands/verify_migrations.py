...
from django.db.migrations.operations import (
    RemoveField, DeleteModel, RenameField, RenameModel, AddField, AddIndex
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write("[PropertyOS] Auditing migrations for zero-downtime compliance...")
        
        loader = MigrationLoader(None)
        unsafe_migrations = []

        local_apps = {
            'accounts', 'analytics', 'audit', 'leads', 'media', 'properties', 'sharing', 'whatsapp'
        }
            ...
            for op in migration.operations:
                ...
                if isinstance(op, RemoveField):
                    unsafe_migrations.append(
                        (app_label, migration_name, f"RemoveField: Dropping column '{op.name}' from model '{op.model_name}'")
                    )
                # 2. Dropping Tables
                elif isinstance(op, DeleteModel):
                    unsafe_migrations.append(
                        (app_label, migration_name, f"DeleteModel: Dropping table for model '{op.name}'")
                    )
                # 3. Renaming Columns
                elif isinstance(op, RenameField):
                    unsafe_migrations.append(
                        (app_label, migration_name, f"RenameField: Renaming column '{op.old_name}' to '{op.new_name}' on model '{op.model_name}'")
                    )
                # 4. Renaming Tables
                elif isinstance(op, RenameModel):
                    unsafe_migrations.append(
                        (app_label, migration_name, f"RenameModel: Renaming table for model '{op.old_name}' to '{op.new_name}'")
                    )
                # 5. Non-nullable additions
                    ...
                    if not getattr(field, 'null', False) and getattr(field, 'default', NOT_PROVIDED) is NOT_PROVIDED:
                        # Primary keys / Auto fields are populated automatically
                        from django.db.models import AutoField, UUIDField
                        if not isinstance(field, (AutoField,)) and not (isinstance(field, UUIDField) and getattr(field, 'default', None) is not None):
                            unsafe_migrations.append(
                                (app_label, migration_name, f"AddField: Adding non-nullable field '{op.name}' without a default to '{op.model_name}'")
                            )
