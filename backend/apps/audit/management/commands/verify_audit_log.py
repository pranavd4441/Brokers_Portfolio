...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write("[PropertyOS] Beginning cryptographic audit log verification...")
        
        logs = ImmutableAuditLog.objects.order_by('created_at', 'id')
        total_records = logs.count()
        
        if total_records == 0:
            self.stdout.write(self.style.SUCCESS("No audit records found. Chain is empty and valid."))
            return

        expected_previous_hash = "GENESIS_ROOT_HASH_POINTER"
        signing_key = getattr(settings, 'AUDIT_LOG_SIGNING_KEY', 'insecure-default-audit-key-for-local-runs-123')
        ...
        for index, log in enumerate(logs):
            ...
            data_string = f"{log.previous_hash}:{log.payload}:{log.actor_email}:{log.action}"
        ...
        if tampered_records:
            ...
            for record in tampered_records:
                self.stdout.write(self.style.ERROR(f" - Log ID: {record['id']} | Action: {record['action']} | Reason: {record['reason']}"))
            sys.exit(1)
        else:
            self.stdout.write(
                self.style.SUCCESS(
