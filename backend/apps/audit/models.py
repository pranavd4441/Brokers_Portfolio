...
class ImmutableAuditLog(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.current_hash:
            ...
            last_entry = ImmutableAuditLog.objects.order_by('-created_at', '-id').first()
            self.previous_hash = last_entry.current_hash if last_entry else "GENESIS_ROOT_HASH_POINTER"
            ...
            data_string = f"{self.previous_hash}:{self.payload}:{self.actor_email}:{self.action}"
