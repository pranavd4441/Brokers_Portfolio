...
def test_lead_api_tenant_isolation(api_client, test_setup):
    ...
    patch_response = api_client.patch(
        f"/api/leads/{lead_a.id}/",
        {"status": "CONTACTED", "notes": "Spoke to buyer, scheduling visit"},
    )
...
def test_whatsapp_nlp_lead_commands(mock_gateway_fn, api_client, test_setup):
    ...
    ShareLink.objects.create(property=property_obj, created_by=user_a, tenant=tenant_a)
    ...
    response = api_client.post(
        webhook_url, {"From": from_number, "Body": "new leads today", "NumMedia": "0"}
    )
    assert response.status_code == 200
    ...
    response = api_client.post(
        webhook_url,
        {"From": from_number, "Body": "leads for Bandra Mansion", "NumMedia": "0"},
    )
    assert response.status_code == 200
