...
from apps.accounts.models import Tenant, UserSession, PasswordHistory, MFATicket, MFARecoveryCode
...
class TestEnterpriseSecurity:
    ...
    def test_password_history_saving_and_checking(self):
        ...
        assert any(check_password("NewPassword123!", h.password_hash) for h in histories)
        assert any(check_password("EnterprisePassword123!", h.password_hash) for h in histories)
    ...
    def test_user_session_logout_endpoints(self):
        ...
        s1 = UserSession.objects.create(user=self.user, token_jti="jti-1", browser="Chrome", os="Windows", is_active=True)
        s2 = UserSession.objects.create(user=self.user, token_jti="jti-2", browser="Firefox", os="Linux", is_active=True)
        ...
        response_other = self.client.post(
            logout_other_url, 
            HTTP_USER_AGENT="Firefox Testing", 
    ...
    def test_mfa_verification_workflow(self):
        ...
        response_wrong = self.client.post(verify_url, {
            "ticket": ticket_id,
            "code": "111111"
        })
        ...
        with patch.object(pyotp.TOTP, 'verify', return_value=True):
            response_ok = self.client.post(verify_url, {
                "ticket": ticket_id,
                "code": "123456"
            })
