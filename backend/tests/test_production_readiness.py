...
from property_os.logging import JSONFormatter, get_log_context, set_log_context, clear_log_context
...
class TestProductionReadiness:
    ...
    def test_global_exception_middleware(self):
        ...
        with patch.object(HealthCheckView, 'get', side_effect=Exception("Centralized crash test")):
            response = self.client.get('/api/health/')
    ...
    def test_feature_flags_registration_disabled(self):
        ...
        with patch.object(FeatureFlagService, 'is_enabled', side_effect=lambda flag: False if flag == "ENABLE_REGISTRATION" else True):
    ...
    def test_rate_limiting_enforcement(self):
        ...
        with patch.object(AnonRateThrottle, 'allow_request', return_value=False):
            with patch.object(AnonRateThrottle, 'wait', return_value=60):
                response = self.client.post(login_url, {"email": "test@example.com", "password": "wrongpassword"})
                assert response.status_code == 429
                assert 'Retry-After' in response.headers


    def test_environment_validation_fail_fast(self):
        with patch.dict('os.environ', {'DJANGO_ENVIRONMENT': 'production', 'DJANGO_SECRET_KEY': 'django-insecure-key'}):
