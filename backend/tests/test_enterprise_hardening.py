...
from apps.accounts.quotas import QuotaService
from apps.audit.models import ImmutableAuditLog
from property_os.encryption import EncryptedCharField, FieldEncryptor
...
class TestEnterpriseHardening:
    ...
    def setup_data(self):
        ...
        self.tenant = Tenant.objects.create(
            name="Testing Workspace Plan Limit", subscription_plan="FREE"
        )
    ...
    def test_secrets_manager_lazy_caching(self):
        with patch(
            "property_os.secrets.factory.SecretProviderFactory.get_provider"
        ) as mock_factory:
    ...
    def test_immutable_audit_log_hash_chain(self):
        ...
        ImmutableAuditLog.objects.filter(id=log1.id).update(
            payload='{"id": "prop-1", "name": "Hacked Mansion"}'
        )
    ...
    def test_tenant_plan_quotas(self):
        ...
        u2 = User.objects.create_user(
            email="user2@example.com", password="Password123!", tenant=self.tenant
        )
    ...
    def test_progressive_feature_flags_targeting(self):
        ...
        with patch.dict(
            "os.environ", {"FEATURE_OVERRIDE_ENABLE_WHATSAPP_USERS": str(self.user.id)}
        ):
            assert (
                FeatureFlagService.is_enabled(
                    "ENABLE_WHATSAPP", {"user_id": str(self.user.id)}
                )
                is True
            )
        ...
        with patch.dict("os.environ", {"FEATURE_ROLLOUT_ENABLE_WHATSAPP": "50"}):
            ...
            h2 = (
                int(hashlib.md5(f"{tenant2}:ENABLE_WHATSAPP".encode()).hexdigest(), 16)
                % 100
            )  # nosec B324

            res1 = FeatureFlagService.is_enabled(
                "ENABLE_WHATSAPP", {"tenant_id": tenant1}
            )
            res2 = FeatureFlagService.is_enabled(
                "ENABLE_WHATSAPP", {"tenant_id": tenant2}
            )
    ...
    def test_structured_error_api_exceptions(self):
        ...
        response = self.client.get(
            "/api/v1/auth/sessions/99999999-9999-9999-9999-999999999999/logout/"
        )
