...
for u in User.objects.all():
    print(f"ID: {u.id} | Email: {u.email} | Name: {u.name} | Phone: {u.phone} | Tenant: {u.tenant.name if u.tenant else 'None'}")

print("\n=== PROPERTIES ===")
for p in Property.objects_unfiltered.all():
    print(f"ID: {p.id} | Title: {p.title} | Price: {p.price} | Created By: {p.created_by.email if p.created_by else 'None'}")

print("\n=== SHARE LINKS ===")
for s in ShareLink.objects_unfiltered.all():
    print(f"ID: {s.id} | Slug: {s.slug} | Property ID: {s.property_id} | Created By: {s.created_by.email if s.created_by else 'None'}")
