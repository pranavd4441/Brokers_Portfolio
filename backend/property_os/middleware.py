...
class TenantContextLoggingMiddleware(MiddlewareMixin):
    ...
    def process_response(self, request, response):
        ...
        tenant_id = getattr(request, 'tenant_id', None) or (user.tenant_id if user and hasattr(user, 'tenant_id') else None)
...
class GlobalExceptionMiddleware:
    ...
    def handle_exception(self, request, exception):
        ...
        content_length = request.headers.get('Content-Length') or request.META.get('CONTENT_LENGTH')
        ...
        tenant_id = getattr(request, 'tenant_id', None) or (str(user.tenant_id) if user and getattr(user, 'tenant_id', None) else None)
...
class ApiPayloadLimitMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        if request.path.startswith('/api/'):
            content_length_str = request.headers.get('Content-Length') or request.META.get('CONTENT_LENGTH')
...
class ContentTypeValidationMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        if request.method in ('POST', 'PUT', 'PATCH') and request.path.startswith('/api/'):
            ...
            if not content_type:
                return JsonResponse({"detail": "Content-Type header is required."}, status=415)
