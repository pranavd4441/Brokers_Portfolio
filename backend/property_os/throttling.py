    ...
    def allow_request(self, request, view):
        ...
        if cache.get(blacklist_key):
            security_logger.warning(
                f"Request blocked: IP {ip_addr} is currently blacklisted."
            )
        ...
        if redis_client:
            try:
                ...
                logger.warning(
                    f"Redis sliding window failed, falling back to simple cache: {str(e)}"
                )
        ...
        try:
            ...
            if count >= 10:
                ...
                security_logger.error(
                    f"IP {ip_addr} has been auto-blacklisted due to excessive rate limiting violations."
                )
        except Exception:
            pass  # nosec B110
...
def custom_exception_handler(exc, context):
    ...
    if response is not None:
        from property_os.errors import format_error_payload, resolve_exception_details
