    ...
    def is_enabled(cls, flag_name: str, context: dict = None) -> bool:
        ...
        if context:
            ...
            if target_roles and role:
                if str(role).upper() in [
                    r.strip().upper() for r in target_roles.split(",")
                ]:
                    return True
        ...
        if context:
            ...
            if target_id:
                ...
                hash_score = (
                    int(
                        hashlib.md5(f"{target_id}:{flag_name}".encode()).hexdigest(), 16
                    )
                    % 100
                )  # nosec B324
                ...
                if rollout_str:
                    try:
                        ...
                        if hash_score < rollout_percent:
                            ...
                            return (
                                False  # Target is excluded from this percentage segment
                            )
                    except ValueError:
                        pass
